...
# d2 = date(1989, 4, 21)  # end date

def get_comic_strip_end_date():
	print("Type a dilbert comic strip date in YYYY/MM/DD format:")
	while True:
		start_year, start_month, start_day = map(int, input(">> ").split("/"))
		end_date = date(start_year, start_month, start_day)
...
LAST_COMIC = get_comic_strip_end_date()

start = time.time()

url_list = get_comic_strip_url(FIRST_COMIC, LAST_COMIC)
